...
impl FromIterator<BaseField> for BaseColumn {
    fn from_iter<I: IntoIterator<Item = BaseField>>(iter: I) -> Self {
        let values: Vec<BaseField> = iter.into_iter().collect();
        let length = values.len();
        let mut data = Vec::with_capacity(length.div_ceil(N_LANES));
        for chunk in values.chunks(N_LANES) {
            let mut packed = [BaseField::zero(); N_LANES];
            packed[..chunk.len()].copy_from_slice(chunk);
            data.push(PackedBaseField::from_array(packed));
        }
        Self { data, length }
    }
}
...
impl FromIterator<CM31> for CM31Column {
    fn from_iter<I: IntoIterator<Item = CM31>>(iter: I) -> Self {
        let values: Vec<CM31> = iter.into_iter().collect();
        let length = values.len();
        let mut data = Vec::with_capacity(length.div_ceil(N_LANES));
        for chunk in values.chunks(N_LANES) {
            let mut packed = [CM31::zero(); N_LANES];
            packed[..chunk.len()].copy_from_slice(chunk);
            data.push(PackedCM31::from_array(packed));
        }
        Self { data, length }
    }
}
...
impl FromIterator<SecureField> for SecureColumn {
    fn from_iter<I: IntoIterator<Item = SecureField>>(iter: I) -> Self {
        let values: Vec<SecureField> = iter.into_iter().collect();
        let length = values.len();
        let mut data = Vec::with_capacity(length.div_ceil(N_LANES));
        for chunk in values.chunks(N_LANES) {
            let mut packed = [SecureField::zero(); N_LANES];
            packed[..chunk.len()].copy_from_slice(chunk);
            data.push(PackedSecureField::from_array(packed));
        }
        Self { data, length }
    }
}
