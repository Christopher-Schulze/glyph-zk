...
pub fn make_sumcheck_claim_descs<F: Field>(
	...
	let mut composite_counts = vec![0usize; commit_meta.max_n_vars() + 1];
	for (i, claim) in claims.iter().enumerate() {
		if claim.n_vars >= composite_counts.len() {
			bail!(Error::SumcheckClaimVariablesMismatch { index: i });
		}
		composite_counts[claim.n_vars] += 1;
	}
	let mut transparent_counts = vec![0usize; commit_meta.max_n_vars() + 1];
	let mut last_n_vars = 0usize;
	for transparent_n_vars in transparent_n_vars_iter {
		if transparent_n_vars >= transparent_counts.len() {
			return Err(Error::TransparentsNotSorted);
		}
		if transparent_n_vars < last_n_vars {
			return Err(Error::TransparentsNotSorted);
		}
		transparent_counts[transparent_n_vars] += 1;
		last_n_vars = transparent_n_vars;
	}
	...
	for (n_vars, claim_desc) in sumcheck_claim_descs.iter_mut().enumerate() {
		claim_desc.composite_sums = Vec::with_capacity(composite_counts[n_vars]);
	}
	let mut transparent_offset = 0usize;
	for (n_vars, count) in transparent_counts.into_iter().enumerate() {
		let next = transparent_offset + count;
		sumcheck_claim_descs[n_vars].transparent_indices = transparent_offset..next;
		transparent_offset = next;
	}
...
pub fn make_sumcheck_claim_descs_with_committed<F: Field>(
	committed_ranges: &[Range<usize>],
	transparent_n_vars_iter: impl Iterator<Item = usize>,
	claims: &[PIOPSumcheckClaim<F>],
) -> Result<Vec<SumcheckClaimDesc<F>>, Error> {
	let mut composite_counts = vec![0usize; committed_ranges.len()];
	for (i, claim) in claims.iter().enumerate() {
		if claim.n_vars >= composite_counts.len() {
			bail!(Error::SumcheckClaimVariablesMismatch { index: i });
		}
	...
	let mut transparent_counts = vec![0usize; committed_ranges.len()];
	let mut last_n_vars = 0usize;
	for transparent_n_vars in transparent_n_vars_iter {
		if transparent_n_vars >= transparent_counts.len() {
			return Err(Error::TransparentsNotSorted);
	...
	let mut sumcheck_claim_descs = vec![
		...
		committed_ranges.len()
	];

	for (n_vars, committed_indices) in committed_ranges.iter().cloned().enumerate() {
		let desc = &mut sumcheck_claim_descs[n_vars];
		desc.committed_indices = committed_indices;
		desc.composite_sums = Vec::with_capacity(composite_counts[n_vars]);
	}
	let mut transparent_offset = 0usize;
	for (n_vars, count) in transparent_counts.into_iter().enumerate() {
		let next = transparent_offset + count;
	...
	for (i, claim) in claims.iter().enumerate() {
		if claim.n_vars >= sumcheck_claim_descs.len() {
			bail!(Error::SumcheckClaimVariablesMismatch { index: i });
		}
		let claim_desc = &mut sumcheck_claim_descs[claim.n_vars];
