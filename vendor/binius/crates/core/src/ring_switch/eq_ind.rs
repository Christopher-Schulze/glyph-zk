	...
	F: ExtensionField<FSub>,
{
	...
	pub fn precompute_values_with_row_batching<
		'a,
		Hal: ComputeLayer<F>,
		HostAllocatorType,
		DeviceAllocatorType,
	>(
		...
		mixing_coeff: F,
		kappa: usize,
		row_batching_query_expansion: <Hal::DevMem as ComputeMemory<F>>::FSlice<'a>,
		hal: &Hal,
		dev_alloc: &'a DeviceAllocatorType,
		host_alloc: &HostAllocatorType,
	) -> Result<RingSwitchEqIndPrecompute<'a, F, Hal::DevMem>, Error>
	...
	{
		let extension_degree = 1 << (kappa);
		if row_batch_coeffs.coeffs.len() < extension_degree {
			bail!(Error::InvalidArgs(
				"RingSwitchEqInd::precompute_values_with_row_batching expects row_batch_coeffs length greater than or equal to \
				the extension degree"
					.into()
			));
		}
