	...
	F: TowerField,
{
	...
	fn hash_eval_point<F: Hash>(eval_point: &[F]) -> u64 {
		let mut hasher = DeterministicHasher(0xcbf29ce484222325);
		eval_point.hash(&mut hasher);
		hasher.finish()
	}

	let mut suffix_oracle_id = Vec::new();

	for (idx, (claim, meta)) in projected_bivariate_claims
		.iter()
		.zip(metas.iter())
		.enumerate()
	{
		if meta.projected_id.is_some() {
			...
			let hash = hash_eval_point(suffix);
			suffix_oracle_id.push((hash, idx, suffix, meta.inner_id));
		}
	}

	suffix_oracle_id.sort_by(|(ha, ia, _, _), (hb, ib, _, _)| ha.cmp(hb).then_with(|| ia.cmp(ib)));
	suffix_oracle_id.dedup_by(|(ha, _, a, a_id), (hb, _, b, b_id)| {
		*ha == *hb && *a == *b && a_id == b_id
	});
	suffix_oracle_id.sort_by(|(_, ia, _, _), (_, ib, _, _)| ia.cmp(ib));

	memoized_queries.memoize_query_par(
		suffix_oracle_id
			.iter()
			.map(|(_, _, suffix, _)| *suffix),
	)?;

	let suffix_oracle_id = suffix_oracle_id
		.into_iter()
		.map(|(_, _, suffix, inner_id)| (suffix, inner_id))
		.collect::<Vec<_>>();
...
impl<'a, P: PackedField> MemoizedData<'a, P> {
	...
	pub fn memoize_query_par<'b>(
		...
		let mut candidates = eval_points
			.into_iter()
			.enumerate()
			.map(|(idx, ep)| (hash_eval_point(ep), idx, ep))
			.collect::<Vec<_>>();
		candidates.sort_by(|(ha, ia, _), (hb, ib, _)| ha.cmp(hb).then_with(|| ia.cmp(ib)));
		candidates.dedup_by(|(ha, _, a), (hb, _, b)| *ha == *hb && *a == *b);

		if self.query.is_empty() {
			let mut new_queries = candidates
				.into_par_iter()
				.map(|(_, idx, ep)| {
			...
			new_queries.sort_by(|(ia, _, _), (ib, _, _)| ia.cmp(ib));
			self.query.reserve(new_queries.len());
			self.query
				.extend(new_queries.into_iter().map(|(_, ep, query)| (ep, query)));
			return Ok(());
		}
		...
		let mut existing = self
			.query
			.iter()
			.map(|(ep, _)| (hash_eval_point(ep.as_slice()), ep.as_slice()))
			.collect::<Vec<_>>();
		existing.sort_by(|(a, _), (b, _)| a.cmp(b));

		let new_queries = candidates
			.into_par_iter()
			.filter(|(hash, _, ep)| {
				match existing.binary_search_by(|(probe_hash, _)| probe_hash.cmp(hash)) {
					Ok(mut idx) => {
						while idx > 0 && existing[idx - 1].0 == *hash {
							idx -= 1;
						}
						let mut scan = idx;
						while scan < existing.len() && existing[scan].0 == *hash {
							if existing[scan].1 == *ep {
								return false;
							}
							scan += 1;
						}
		...
		let mut new_queries = new_queries;
		new_queries.sort_by(|(ia, _, _), (ib, _, _)| ia.cmp(ib));

		self.query.reserve(new_queries.len());
		self.query.extend(new_queries.into_iter().map(|(_, ep, query)| (ep, query)));
