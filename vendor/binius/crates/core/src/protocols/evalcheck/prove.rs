...
use binius_utils::{SerializeBytes, SerializationMode};
use bytes::BytesMut;
...
fn eval_point_sort_key<F: Field>(eval_point: &EvalPoint<F>) -> Vec<u8> {
	let mut buf = BytesMut::with_capacity(eval_point.len().saturating_mul(32));
	for value in eval_point.as_ref() {
		value
			.serialize(&mut buf, SerializationMode::CanonicalTower)
			.expect("eval point serialization failed");
	}
	buf.to_vec()
}

fn sort_dedup_eval_points<F: Field>(points: &mut Vec<EvalPoint<F>>) {
	if points.len() <= 1 {
		return;
	}
	let mut keyed = points
		.drain(..)
		.map(|ep| (eval_point_sort_key(&ep), ep))
		.collect::<Vec<_>>();
	keyed.sort_by(|(ka, _), (kb, _)| ka.cmp(kb));
	keyed.dedup_by(|(ka, _), (kb, _)| ka == kb);
	points.extend(keyed.into_iter().map(|(_, ep)| ep));
}
	...
	F: TowerField,
{
	...
	pub fn prove<Challenger_: Challenger>(
		...
		let mut eval_points = self
			...
			.collect::<Vec<_>>();
		sort_dedup_eval_points(&mut eval_points);

		let mut suffixes = self.suffixes.iter().cloned().collect::<Vec<_>>();
		sort_dedup_eval_points(&mut suffixes);

		let mut prefixes = Vec::new();
		let mut remaining_eval_points = Vec::with_capacity(eval_points.len());
		for eval_point in &eval_points {
			...
			if !matched {
				remaining_eval_points.push(eval_point.clone());
			}
		}
		eval_points = remaining_eval_points;

		// We don't split points whose tensor product, when halved, would be smaller than
		// PackedField::WIDTH.
		let (long, mut short): (Vec<_>, Vec<_>) = eval_points
			.into_iter()
		...
		sort_dedup_eval_points(&mut suffixes);
		sort_dedup_eval_points(&mut prefixes);
		sort_dedup_eval_points(&mut short);
		...
		let mut claims_to_be_evaluated = std::mem::take(&mut self.claims_to_be_evaluated)
			.into_iter()
			.map(|(id, eval_point)| (id, eval_point_sort_key(&eval_point), eval_point))
			.collect::<Vec<_>>();
		claims_to_be_evaluated
			.sort_by(|(id_a, key_a, _), (id_b, key_b, _)| id_a.cmp(id_b).then_with(|| key_a.cmp(key_b)));
		let subclaims_partial_evals = claims_to_be_evaluated
			.into_par_iter()
			.map(|(id, _, eval_point)| {
		...
		let mut claims_without_evals = std::mem::take(&mut self.claims_without_evals)
			...
			.map(|(id, eval_point)| (id, eval_point_sort_key(&eval_point), eval_point))
			.collect::<Vec<_>>();
		claims_without_evals.sort_by(|(id_a, key_a, _), (id_b, key_b, _)| {
			id_a.cmp(id_b).then_with(|| key_a.cmp(key_b))
		});

		for (id, _, eval_point) in claims_without_evals {
