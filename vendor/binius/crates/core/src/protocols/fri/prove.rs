...
pub fn commit_interleaved_with_buffer<F, FA, P, PA, NTT, MerkleProver, VCS>(
	params: &FRIParams<F, FA>,
	ntt: &NTT,
	merkle_prover: &MerkleProver,
	encoded: &mut Vec<P>,
...
where
	F: BinaryField,
	FA: BinaryField,
	P: PackedField<Scalar = F> + PackedExtension<FA, PackedSubfield = PA> + Zeroable,
	PA: PackedField<Scalar = FA>,
	NTT: AdditiveNTT<FA> + Sync,
	MerkleProver: MerkleTreeProver<F, Scheme = VCS>,
	VCS: MerkleTreeScheme<F>,
{
	...
	let total_len = 1 << (log_elems - P::LOG_WIDTH + rs_code.log_inv_rate());
	if encoded.len() != total_len {
		encoded.resize(total_len, P::zeroed());
	}
	encoded.fill(P::zeroed());
	...
	.in_scope(|| rs_code.encode_ext_batch_inplace(ntt, encoded, log_batch_size))?;
	...
	let codeword = std::mem::take(encoded);
	Ok(CommitOutput {
		commitment: commitment.root,
		committed: vcs_committed,
		codeword,
		...
		let folded_codeword = match self.round_committed.last() {
			...
			None => {
				...
				let mut original_codeword = allocator.alloc(codeword_len)?;
                                let scalars = unpack_if_possible(
                                        self.codeword,
                                        |scalars| scalars.to_vec(),
                                        |packed| PackedField::iter_slice(packed).collect::<Vec<_>>(),
                                );
