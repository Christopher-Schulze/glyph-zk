	...
	Backend: ComputationBackend,
{
	...
	if composition_max_degree <= 1 {
		let partial_eq_ind_evals: <Backend as ComputationBackend>::Vec<P> =
			backend.tensor_product_full_query(zerocheck_challenges)?;
		let extrapolated_len = max_domain_size.saturating_sub(1 << skip_rounds);
		let round_evals = compositions
			.iter()
			.map(|_| vec![F::ZERO; extrapolated_len])
			.collect::<Vec<_>>();
		return Ok(ZerocheckUnivariateEvalsOutput {
			round_evals,
			skip_rounds,
			remaining_rounds,
			max_domain_size,
