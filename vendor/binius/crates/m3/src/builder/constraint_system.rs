...
use binius_core::{
	...
	transparent::challenge::Challenge,
};
...
impl<F: TowerField> ConstraintSystem<F> {
	...
	pub fn compile(&self) -> Result<CompiledConstraintSystem<F>, Error> {
		...
		let mut challenge_offsets = Vec::with_capacity(self.tables.len());
		...
		let challenge_count = next_challenge_id;
...
fn add_oracle_for_column<F: TowerField>(
	...
	match col {
		...
		ColumnDef::Challenge { challenge_id } => {
			let global_id = challenge_offset
				.checked_add(*challenge_id)
				.expect("challenge id overflow");
			let poly = Challenge::<F>::new(global_id, shape.tower_height);
