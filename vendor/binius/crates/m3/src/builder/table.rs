...
impl<'a, F: TowerField> TableBuilder<'a, F> {
	...
	/// Adds a per-proof challenge column backed by the transcript.
	pub fn add_challenge(&mut self, name: impl ToString) -> Col<B128>
	where
		F: ExtensionField<B128>,
	{
		let challenge_id = self.table.alloc_challenge_id();
		self.table
			.new_column(self.namespaced_name(name), ColumnDef::Challenge { challenge_id })
	}
...
impl<F: TowerField> Table<F> {
	...
	fn alloc_challenge_id(&mut self) -> u32 {
		let id = self.challenge_count;
		self.challenge_count = self
			.challenge_count
			.checked_add(1)
