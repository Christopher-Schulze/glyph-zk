...
use binius_core::{
	transparent::challenge::challenge_value,
...
#[derive(Debug, Clone)]
struct ChallengeMultilinear<F: TowerField> {
	n_vars: usize,
	challenge_id: u32,
	_marker: PhantomData<F>,
}

impl<F: TowerField> ChallengeMultilinear<F> {
	fn new(n_vars: usize, challenge_id: u32) -> Self {
		Self {
			n_vars,
			challenge_id,
			_marker: PhantomData,
	...
	fn value<P: PackedField<Scalar = F>>(&self) -> Result<F, binius_math::Error> {
		challenge_value::<F>(self.challenge_id as usize)
			.map_err(|err| binius_math::Error::PolynomialError(Box::new(err)))
	}

	fn constant_extension<P: PackedField<Scalar = F>>(
		&self,
		n_vars: usize,
	) -> Result<MultilinearExtension<P>, binius_math::Error> {
		let value = self.value::<P>()?;
		let len = 1usize << n_vars.saturating_sub(P::LOG_WIDTH);
		let packed = P::broadcast(value);
		MultilinearExtension::new(n_vars, vec![packed; len])
	}

	fn extension<P: PackedField<Scalar = F>>(&self) -> Result<MultilinearExtension<P>, binius_math::Error> {
		self.constant_extension::<P>(self.n_vars)
	}
}

impl<F, P> MultilinearPoly<P> for ChallengeMultilinear<F>
where
	F: TowerField + Copy + 'static,
	P: PackedField<Scalar = F>,
{
	...
	fn evaluate_on_hypercube(&self, index: usize) -> Result<F, binius_math::Error> {
		let _ = index;
		self.value::<P>()
	}
	...
	fn evaluate_on_hypercube_and_scale(
		...
		index: usize,
		scalar: F,
	) -> Result<F, binius_math::Error> {
		let _ = index;
		let eval = self.value::<P>()?;
		Ok(eval * scalar)
	}

	fn evaluate(&self, query: binius_math::MultilinearQueryRef<P>) -> Result<F, binius_math::Error> {
		if query.n_vars() != self.n_vars {
	...
	fn evaluate_partial_low(
		&self,
		query: binius_math::MultilinearQueryRef<P>,
	) -> Result<MultilinearExtension<P>, binius_math::Error> {
		self.extension::<P>()?.evaluate_partial_low(query)
	}
	...
	fn evaluate_partial_high(
		...
		self.extension::<P>()?.evaluate_partial_high(query)
	}
	...
	fn evaluate_partial(
		...
		self.extension::<P>()?.evaluate_partial(query, start_index)
	}
	...
	fn zero_pad(
		...
		self.extension::<P>()?
			.zero_pad(n_pad_vars, start_index, nonzero_index)
	}
	...
	fn subcube_partial_low_evals(
		...
		if query_n_vars + subcube_vars > self.n_vars {
			return Err(binius_math::Error::ArgumentRangeError {
				arg: "query.n_vars() + subcube_vars".to_string(),
				range: 0..self.n_vars,
			});
		}
		let max_index = 1usize << (self.n_vars - query_n_vars - subcube_vars);
		...
		let correct_len = 1usize << subcube_vars.saturating_sub(P::LOG_WIDTH);
		if partial_low_evals.len() != correct_len {
			return Err(binius_math::Error::ArgumentRangeError {
		...
		let value = self.value::<P>()?;
		let packed = P::broadcast(value);
		for slot in partial_low_evals.iter_mut() {
			*slot = packed;
		}
		Ok(())
	}
	...
	fn subcube_partial_high_evals(
		...
		let correct_len = 1usize << subcube_vars.saturating_sub(P::LOG_WIDTH);
		if partial_high_evals.len() != correct_len {
			return Err(binius_math::Error::ArgumentRangeError {
				arg: "partial_evals.len()".to_string(),
				range: correct_len..correct_len + 1,
			});
		...
		let value = self.value::<P>()?;
		let packed = P::broadcast(value);
		for slot in partial_high_evals.iter_mut() {
			*slot = packed;
		}
		Ok(())
	}
	...
	fn subcube_evals(
		...
		if log_embedding_degree != 0 {
			return Err(binius_math::Error::LogEmbeddingDegreeTooLarge {
		...
		let max_index = 1usize << (n_vars - subcube_vars);
		if subcube_index >= max_index {
			return Err(binius_math::Error::ArgumentRangeError {
				arg: "subcube_index".to_string(),
				range: 0..max_index,
...
impl<'cs, 'alloc, F: TowerField, P: PackedField<Scalar = F>> WitnessIndex<'cs, 'alloc, P> {
			...
			+ PackedExtension<B128>,
	{
		...
		let mut challenge_offsets = Vec::with_capacity(self.cs.tables.len());
		let mut next_challenge_id: u32 = 0;
		for table in &self.cs.tables {
			challenge_offsets.push(next_challenge_id);
			next_challenge_id = next_challenge_id
				.checked_add(table.challenge_count())
				.expect("challenge id overflow");
		}
		...
		for table_witness in self.tables {
			...
			let mut challenge_ids: BTreeMap<ColumnId, u32> = BTreeMap::new();
			let table_offset = challenge_offsets
				.get(table.id)
				.copied()
				.unwrap_or(0);
			for column in table.columns.iter() {
				if let ColumnDef::Challenge { challenge_id } = &column.col {
					let global_id = table_offset
						.checked_add(*challenge_id)
						.expect("challenge id overflow");
					challenge_ids.insert(column.id, global_id);
				}
			}
			...
			for col in cols.into_iter() {
				...
				match oracle_mapping {
					...
					OracleMapping::TransparentCompound {
						...
						if let Some(global_id) = challenge_ids.get(&col.column_id).copied() {
							let original_vars = col.shape.log_values_per_row;
							let repeating_vars = table_witness.log_capacity + col.shape.log_values_per_row;
							let original_witness: MultilinearWitness<'_, P> = Arc::new(
								ChallengeMultilinear::<P::Scalar>::new(original_vars, global_id),
							);
							let repeating_witness: MultilinearWitness<'_, P> = Arc::new(
								ChallengeMultilinear::<P::Scalar>::new(repeating_vars, global_id),
							);
							index
								.update_multilin_poly([
