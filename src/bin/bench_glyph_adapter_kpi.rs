...
use glyph::glyph_core::{prove_compiled, ProverConfig};
...
fn main() -> Result<(), String> {
    let adapters = std::env::var("GLYPH_ADAPTER_KPI")
    ...
    let ic_window = env_usize("GLYPH_ADAPTER_KPI_GROTH16_BN254_IC_WINDOW", 6);
    let use_groth16_precomp = env_bool("GLYPH_ADAPTER_KPI_GROTH16_BN254_PRECOMP", true);
    let use_kzg_precomp = env_bool("GLYPH_ADAPTER_KPI_KZG_BN254_PRECOMP", true);
    let use_stark_f64 = env_bool("GLYPH_ADAPTER_KPI_STARK_F64", false);
    let use_stark_circle = env_bool("GLYPH_ADAPTER_KPI_STARK_CIRCLE", false);
    let use_stark_circle_large = env_bool("GLYPH_ADAPTER_KPI_STARK_CIRCLE_LARGE", false);
    let use_stark_baby_bear = env_bool("GLYPH_ADAPTER_KPI_STARK_BABY_BEAR", false);
    let prove_repeat = env_usize("GLYPH_ADAPTER_KPI_PROVE_REPEAT", 1).max(1);

    if do_groth16 {
        let (raw_vk_bytes, raw_proof_bytes, raw_pub_bytes) =
            load_groth16_bn254_fixture_bytes().map_err(|e| format!("groth16 fixture: {e}"))?;
        let input_layout_hash = keccak256(b"glyph-groth16-bn254-kpi-layout");
        ...
        let (arith, copy, custom) = compiled.ucir.gate_counts();
        let layout = compiled.ucir.witness_layout.clone();
        let ucir_bytes = compiled.ucir.to_bytes().len();

        let mut prove_ms = 0u128;
        let mut calldata_bytes = 0usize;
        for _ in 0..prove_repeat {
            let prove_start = Instant::now();
            let proof = prove_compiled(clone_compiled(&compiled), ProverConfig::default())
                .map_err(|e| format!("groth16-bn254 prove failed: {e:?}"))?;
            prove_ms += prove_start.elapsed().as_millis();
            calldata_bytes = proof.packed_gkr_calldata.len();
        }
        let prove_avg_ms = prove_ms / prove_repeat as u128;

        println!(
            "{{\"kpi\":\"GLYPH_ADAPTER_KPI\",\"adapter\":\"SNARK_GROTH16_BN254\",\"compile_ms\":{},\"prove_ms\":{},\"prove_avg_ms\":{},\"prove_repeat\":{},\"total_ms\":{},\"arith_gates\":{},\"copy_gates\":{},\"custom_gates\":{},\"public_len\":{},\"wire_len\":{},\"lookup_len\":{},\"ucir_bytes\":{},\"calldata_bytes\":{},\"vk_hash\":\"{}\"}}",
            compile_ms,
            prove_ms,
            prove_avg_ms,
            prove_repeat,
    ...
    if do_kzg {
        ...
        let input_layout_hash = keccak256(b"glyph-kzg-bn254-kpi-layout");
        let vk_hash = keccak256(&raw_vk_bytes);
        ...
        for _ in 0..prove_repeat {
            let prove_start = Instant::now();
            let proof = prove_compiled(clone_compiled(&compiled), ProverConfig::default())
                .map_err(|e| format!("kzg-bn254 prove failed: {e:?}"))?;
            prove_ms += prove_start.elapsed().as_millis();
            calldata_bytes = proof.packed_gkr_calldata.len();
        }
        let prove_avg_ms = prove_ms / prove_repeat as u128;
        ...
        println!(
            "{{\"kpi\":\"GLYPH_ADAPTER_KPI\",\"adapter\":\"SNARK_KZG_BN254\",\"compile_ms\":{},\"prove_ms\":{},\"prove_avg_ms\":{},\"prove_repeat\":{},\"total_ms\":{},\"arith_gates\":{},\"copy_gates\":{},\"custom_gates\":{},\"public_len\":{},\"wire_len\":{},\"lookup_len\":{},\"ucir_bytes\":{},\"calldata_bytes\":{},\"vk_hash\":\"{}\"}}",
            compile_ms,
            prove_ms,
            prove_avg_ms,
            prove_repeat,
    ...
    if do_ivc {
        ...
        for _ in 0..prove_repeat {
            let prove_start = Instant::now();
            let proof = prove_compiled(clone_compiled(&compiled), ProverConfig::default())
                .map_err(|e| format!("ivc prove failed: {e:?}"))?;
            prove_ms += prove_start.elapsed().as_millis();
            calldata_bytes = proof.packed_gkr_calldata.len();
        }
        let prove_avg_ms = prove_ms / prove_repeat as u128;
        ...
        println!(
            "{{\"kpi\":\"GLYPH_ADAPTER_KPI\",\"adapter\":\"IVC\",\"compile_ms\":{},\"prove_ms\":{},\"prove_avg_ms\":{},\"prove_repeat\":{},\"total_ms\":{},\"arith_gates\":{},\"copy_gates\":{},\"custom_gates\":{},\"public_len\":{},\"wire_len\":{},\"lookup_len\":{},\"ucir_bytes\":{},\"calldata_bytes\":{}}}",
            compile_ms,
            prove_ms,
            prove_avg_ms,
            prove_repeat,
    ...
    if do_stark {
        let seed = b"glyph-stark-kpi-seed";
        ...
        println!(
            "{{\"kpi\":\"GLYPH_ADAPTER_KPI\",\"adapter\":\"STARK\",\"compile_ms\":{},\"prove_ms\":{},\"prove_avg_ms\":{},\"prove_repeat\":{},\"total_ms\":{},\"arith_gates\":{},\"copy_gates\":{},\"custom_gates\":{},\"public_len\":{},\"wire_len\":{},\"lookup_len\":{},\"ucir_bytes\":{},\"calldata_bytes\":{}}}",
            compile_ms,
            prove_ms,
            prove_avg_ms,
            prove_repeat,
    ...
    if do_hash {
        ...
        let bindings = [
            keccak256(b"hash-kpi-bind-1"),
            keccak256(b"hash-kpi-bind-2"),
        ];
        let compile_start = Instant::now();
        ...
        for _ in 0..prove_repeat {
            let prove_start = Instant::now();
            let proof = prove_compiled(clone_compiled(&compiled), ProverConfig::default())
                .map_err(|e| format!("hash prove failed: {e:?}"))?;
            prove_ms += prove_start.elapsed().as_millis();
            calldata_bytes = proof.packed_gkr_calldata.len();
        }
        let prove_avg_ms = prove_ms / prove_repeat as u128;
        ...
        println!(
            "{{\"kpi\":\"GLYPH_ADAPTER_KPI\",\"adapter\":\"HASH\",\"compile_ms\":{},\"prove_ms\":{},\"prove_avg_ms\":{},\"prove_repeat\":{},\"total_ms\":{},\"arith_gates\":{},\"copy_gates\":{},\"custom_gates\":{},\"public_len\":{},\"wire_len\":{},\"lookup_len\":{},\"ucir_bytes\":{},\"calldata_bytes\":{}}}",
            compile_ms,
            prove_ms,
            prove_avg_ms,
            prove_repeat,
