...
fn main() -> Result<(), String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--hash" => {
                ...
                match val.as_str() {
                    "sha3" => hash_override = Some(HASH_SHA3_ID),
                    "blake3" => hash_override = Some(HASH_BLAKE3_ID),
                    "poseidon" => hash_override = Some(HASH_POSEIDON_ID),
                    "rescue" => hash_override = Some(HASH_RESCUE_ID),
    ...
    if let Some(bundle_path) = bundle_json {
        if hash_override.is_some() {
            return Err("bundle-json cannot be combined with --hash".to_string());
        }
        ...
        let input = decode_risc_zero_receipt_input(&bundle_bytes)?;
        let (receipt, program) = match input {
            RiscZeroReceiptInput::Bundle(bundle) => bundle.into_receipt_and_program()?,
            RiscZeroReceiptInput::External(receipt) => receipt.into_receipt_and_program()?,
        };
        let decoded_vk = CanonicalStarkReceipt::decode_and_validate_vk(&receipt)?;
        verify_standard_stark_receipt(&receipt, &decoded_vk, &program)?;
    ...
    let profile = StandardStarkProfile::decode(&profile_bytes)?;
    if profile.log_domain_size == 0 || profile.num_queries == 0 {
        return Err("standard profile invalid".to_string());
    }
    let program = decode_standard_stark_program(&program_bytes)?;
    if let Some(expected) = hash_override {
        if expected != program.hash_id {
            return Err("hash_id mismatch with program".to_string());
        }
    }
...
fn print_help() {
    eprintln!("glyph_import_risc_zero_receipt \\");
    ...
    eprintln!("  [--bundle-json <path>] [--hash <sha3|blake3|poseidon|rescue>] [--out <path>]");
