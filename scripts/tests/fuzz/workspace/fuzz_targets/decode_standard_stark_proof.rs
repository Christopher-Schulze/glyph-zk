...
fuzz_target!(|data: &[u8]| {
    // Exercise standard proof decoding (including nested StandardFriProof decoding).
    use glyph::standard_fri::StandardFriProof;
    use glyph::standard_stark::StandardStarkProof;
    ...
    let _ = StandardStarkProof::decode(&enc);
});
