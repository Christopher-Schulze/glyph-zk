#!/bin/bash
# Bench Groth16 verifier gas on Ethereum Sepolia.
# Uses a small Groth16 circuit generated via snarkjs.
...
DEPLOY_FILE="${DEPLOY_FILE:-$PROJECT_ROOT/deployments/sepolia_groth16.json}"
...
bench_setup_logs "$OUT_DIR/bench_groth16_sepolia.log"
bench_log_basic
...
if [ "$BALANCE" = "0" ]; then
    echo ""
    die_runtime "No Sepolia ETH in wallet"
fi
echo ""
