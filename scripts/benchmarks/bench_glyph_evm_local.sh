...
# Measures full transaction gas (intrinsic + calldata + execution) for several calldata layouts.
...
bench_setup_logs "$OUT_DIR/bench_glyph_evm_local.log"
bench_log_basic
...
echo "=== GLYPHVerifier Bench (Anvil) ==="
...
DEPLOY_GAS="(unknown)"
if [ -n "$DEPLOY_TX" ]; then
    DEPLOY_GAS="$(cast receipt "$DEPLOY_TX" --rpc-url "$RPC_URL" --json | python3 -c 'import json,sys; print(json.load(sys.stdin).get("gasUsed","(unknown)"))')"
fi
...
bench_case() {
    ...
    if [ "$status" != "1" ] && [ "$status" != "0x1" ]; then
        echo "ERROR: tx status != 1 for case=${name}"
        exit 1
    fi
...
bench_case "artifact_full" --hash-merge --seed glyph-test-vector-1 --rounds 5 --full --chainid "$ANVIL_CHAIN_ID" --verifier "$CONTRACT_ADDR"
bench_case "artifact_truncated" --hash-merge --seed glyph-test-vector-1 --rounds 5 --truncated --chainid "$ANVIL_CHAIN_ID" --verifier "$CONTRACT_ADDR"

echo ""

GIT_COMMIT="$(git rev-parse --short HEAD 2>/dev/null || echo "unknown")"
TIMESTAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
python3 - "$RESULTS_TMP" "$OUT_JSON" "$DEPLOY_GAS" "$CONTRACT_ADDR" "$ANVIL_CHAIN_ID" "$RPC_URL" "$GIT_COMMIT" "$TIMESTAMP" <<'PY'
import json
import sys

tmp, out, deploy_gas, contract, chain_id, rpc_url, git_commit, timestamp = sys.argv[1:]
cases = []
