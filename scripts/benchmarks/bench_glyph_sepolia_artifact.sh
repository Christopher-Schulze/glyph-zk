#!/bin/bash
# Bench GLYPHVerifier (V4) gas on Ethereum Sepolia.
...
usage() {
  cat <<'USAGE'
Usage:
  scripts/benchmarks/bench_glyph_sepolia_artifact.sh
...
bench_setup_logs "$OUT_DIR/bench_glyph_sepolia_artifact.log"
bench_log_basic
...
bench_case() {
    ...
    local out
    out="$(target/release/gen_glyph_gkr_proof --artifact-poly --commitment "$COMMITMENT" --point "$POINT" --claim "$CLAIM" --rounds "$ROUNDS" --chainid "$SEPOLIA_CHAIN_ID" --verifier "$CONTRACT_ADDR" "$@" --json)"
    local calldata
...
doc = {
    "bench": "bench_glyph_sepolia_artifact",
    "timestamp": timestamp,
