...
BENCH_BIN="${BENCH_BIN:-$PROJECT_ROOT/target/release/bench_state_diff_merkle}"
BYTES_PATH=""
ITERS="10"
WARMUP="2"
...
usage() {
  cat <<'USAGE'
Usage:
  scripts/benchmarks/bench_state_diff_merkle.sh --bytes <file> [--iters <n>] [--warmup <n>]
...
"$BENCH_BIN" --bytes "$BYTES_PATH" --iters "$ITERS" --warmup "$WARMUP" --json > "$OUT_FILE"
bench_finalize
