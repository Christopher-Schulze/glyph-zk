...
contract GLYPHVerifier is GLYPHVerifierConstants {
    fallback() external payable {
        assembly {
            ...
            // Calldata[0..32): artifact_tag.
            let artifact_tag := calldataload(0)

            let mask := 0xffffffffffffffffffffffffffffffff
            // Calldata[32..64): claim128 (hi 16 bytes) || initial_claim (lo 16 bytes).
            ...
            // Memory scratch [0x00..0x7f]:
            // 0x00..0x1f = chainid
            // 0x20..0x3f = verifier address
            // 0x40..0x5f = artifact_tag
            // 0x60..0x7f = claim_initial (claim||initial)
            mstore(0x00, chainid())
            mstore(0x20, address())
            ...
            // Memory [0xa0..0xef]:
            // 0xa0..0xbf = LIN_DOMAIN
            // 0xc0..0xdf = artifact_tag
            // 0xe0..0xef = claim (16 bytes) as big endian in high half
            ...
            for { } lt(ptr, end) { ptr := add(ptr, 32) } {
                // Calldata[ptr..ptr+32): c0 (hi 16 bytes) || c1 (lo 16 bytes).
                ...
                // Memory scratch [0x00..0x1f]:
                // 0x00..0x0f = r (16 bytes) big endian in high half
                // 0x10..0x1f = mix (16 bytes) in low half
